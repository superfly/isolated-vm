...
void IsolateEnvironment::Scheduler::AsyncCallbackRoot(uv_async_t* async) {
	if (async->data == nullptr) {
		...
		fprintf(stdout, "isolated-vm: %p got terminal ping\n", std::this_thread::get_id());
		return;
	}
	fprintf(stdout, "isolated-vm: %p begin root callback\n", std::this_thread::get_id());
	void* data = async->data;
	async->data = nullptr;
	AsyncCallbackPool(true, data);
	fprintf(stdout, "isolated-vm: %p end root callback\n", std::this_thread::get_id());
}
...
void IsolateEnvironment::Scheduler::AsyncCallbackPool(bool pool_thread, void* param) {
	...
	if (--uv_ref_count == 0) {
		...
		fprintf(stdout, "isolated-vm: %p send terminal ping\n", std::this_thread::get_id());
...
void IsolateEnvironment::Scheduler::Lock::DoneRunning() {
	if (scheduler.default_isolate) {
		fprintf(stdout, "isolated-vm: %p done running\n", std::this_thread::get_id());
	}
...
bool IsolateEnvironment::Scheduler::Lock::WakeIsolate(shared_ptr<IsolateEnvironment> isolate_ptr) {
	if (isolate_ptr->root != scheduler.default_isolate) {
		fprintf(stdout, "isolated-vm: %p default isolate mismatch!\n", std::this_thread::get_id());
	}
	if (scheduler.status == Status::Waiting) {
		...
		if (isolate.root) {
			...
			fprintf(stdout, "isolated-vm: %p send uv async\n", std::this_thread::get_id());
			uv_async_send(&root_async);
		} else {
		...
		if (isolate_ptr->root) {
			if (scheduler.tasks.size() == 10) {
				fprintf(stdout, "isolated-vm: %p skipping uv async, tasks>10\n", std::this_thread::get_id());
			} else if (scheduler.tasks.size() < 10) {
				fprintf(stdout, "isolated-vm: %p skipping uv async, tasks=%lu\n", std::this_thread::get_id(), scheduler.tasks.size());
			}
		}
		return false;
	}
...
void IsolateEnvironment::AsyncEntry() {
	...
	while (true) {
		...
		std::queue<unique_ptr<Runnable>> interrupts;
		{
			...
			if (root) {
				fprintf(stdout, "isolated-vm: %p acquired tasks=%lu\n", std::this_thread::get_id(), tasks.size());
			}
