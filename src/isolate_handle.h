...
class IsolateHandle : public TransferableHandle {
	...
	public:
		 ...
		 */
		class CompileScriptImpl : public ThreePhaseTask {
			private:
				// phase 1
				Local<String>& code_handle;
			...
			public:
				CompileScriptImpl(Local<String>& code_handle, MaybeLocal<Object>& maybe_options, ShareableIsolate* isolate) :
					ThreePhaseTask(), code_handle(code_handle), maybe_options(maybe_options), isolate(isolate),
					produce_cached_data(false), supplied_cached_data(false), cached_data_rejected(false) {}
		...
		Local<Value> CompileScript(Local<String> code_handle, MaybeLocal<Object> maybe_options) {
			return ThreePhaseTask::Run<async, CompileScriptImpl>(this->isolate.get(), code_handle, maybe_options, this->isolate.get());
		}
